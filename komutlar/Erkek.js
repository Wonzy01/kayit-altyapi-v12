...
**Doğru Kullanım;**
> **${ayarlar.prefix}erkek ${message.author} ${message.member.displayName.replace(ayarlar.tag,'').replace(AraÇizgi,'').slice(1)}** 
\`Üye Etiketleyiniz!\``)
if (!Üye) return message.channel.send(Revenge)
if (Üye.roles.cache.has(ayarlar.erkek) && db.has(`KayıtEden_${Üye.id}`)) return message.channel.send(new Discord.MessageEmbed()
.setColor('RED')
...
.setTimestamp()
.addField('\u200b','\u200b')
.addField('Önceki Kullanıcı Adları',await db.fetch(`Nickler_${Üye.id}`).reverse().join(', \n'))
.setFooter(message.author.tag,message.author.avatarURL({dynamic:true}))
message.channel.send(Confirmed)
db.add(`Kayıt_${message.author.id}`,1)
...
exports.help = {
   name: 'Erkek',
   description: 'Erkek Olarak Kayıt Etme.',
   usage: 'erkek'
}
